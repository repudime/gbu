...
###########################################################
potential_transition_vars = ['focalUserReviews', 'qualityOfRestaurantsVisited']
...
def create_LSTM_model_for_fs(n_steps=55, neurons=10, n_features=-1, stacked=False):
    ...
    if stacked:
        model.add(
            LSTM(units=neurons, activation='relu', return_sequences=True))  # , input_shape=(n_steps, n_features)))
        model.add(LSTM(units=int(neurons / 2), activation='relu'))  # , input_shape=(n_steps, n_features)))
    else:
        model.add(LSTM(units=neurons, activation='relu'))  # , input_shape=(n_steps, n_features)))
...
trainTransformed = pd.DataFrame(data=trainedPipeline.transform(train[predictive_variables]),  # values
                                index=None, columns=predictive_variables)
validation_transformed = pd.DataFrame(data=trainedPipeline.transform(validation[predictive_variables]),  # values
                                      index=None, columns=predictive_variables)
test_transformed = pd.DataFrame(data=trainedPipeline.transform(test[predictive_variables]),  # values
                                index=None, columns=predictive_variables)
