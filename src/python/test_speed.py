...
validation = d[d['set_annotation'].str.contains(pat =('validation_'+str(fold))+',|validation_'+str(fold)+"$",regex=True)==True].copy()
trainedPipeline = pipe.fit(train[emission_variables])
trainTransformed = pd.DataFrame(data=trainedPipeline.transform(train[emission_variables]),    # values
             index = None,  columns=emission_variables)
validation_transformed = pd.DataFrame(data=trainedPipeline.transform(validation[emission_variables]),    # values
             index = None,  columns=emission_variables)
...
if __name__ == '__main__':
    ...
    hmm = HMM(transition_application_employer_train,
              ...
              random_seed=1234, n_cores=n_cores, solver="COBYLA", states=3)  # Powell: 1359, for ever.
