...
# d = d.sort_values(['user', 'daysTrend','item_sequential', 'application'])
...
def get_predictions(theta_star, cur_test_X, cur_test_y):
    ...
    variables_in_Z = Z_timelines[0].shape[1]
    variables_in_X = X_timelines[0].shape[1]
    (pi, gammaCoeffsForZ, betaCoeffsforX, sigmaPars) = init_params(theta_star, variables_in_Z, states, emission_distro_pars,
                                                 variables_in_X,
                                                 transition_model)
